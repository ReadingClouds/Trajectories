...
# Sum properties over classes; division by number of points happens later.
        for it in range(0, traj.end+1) :
            ...
            mean_prop[it, iobj, r4] = np.mean(mass[it, obj_ptrs], axis=0)
            mean_prop[it, iobj, r5] = np.mean(grid_box_volume[it, obj_ptrs], axis=0)
            mean_prop[it, iobj, r6] = np.size(data[it, :, 0])
...
#                        print(derv_data[it, lmask, :])
                    ...
                    mean_prop_by_class[it, iobj, r4, iclass] = \
                        np.sum(mass[it, obj_ptrs][lmask], axis=0)
                    mean_prop_by_class[it, iobj, r5, iclass] = \
                        np.sum(grid_box_volume[it, obj_ptrs][lmask], axis=0)
                    mean_prop_by_class[it, iobj, r6, iclass] = \
                        np.size(where_mask)
...
# Cloud volume
        ...
        incl = (n_main_cloud > 0)
        z_cloud = np.interp(v_main_cloud[:, z_ptr], traj.coords['zcoord'],
                            traj.coords['zn'])
