...
from .monc import find_time_in_files
from .utils.geometry import box_overlap_with_wrap
from .constants import r_over_cp
...
class Trajectory_Family:
    ...
    def __init__(
        ...
        first_ref_file, it, delta_t = find_time_in_files(files, first_ref_time)
        ...
        if delta_t > 0.0:
            ...
            print(
                "Starting trajectory family calculation at time {} in file {}".format(
                    first_ref_time, files[first_ref_file]
                )
            )
    ...
    def matching_object_list(self, master_ref=None, select=None):
        ...
        if master_ref is None:
            master_ref = len(self.family) - 1
        traj = self.family[master_ref]
        if select is None:
            select = np.arange(0, traj.nobjects, dtype=int)
                ...
                for iobj in select:
                    ...
                    if traj.num_in_obj[ref_time, iobj] > 0:
                        ...
                        if (match_time >= 0) & (
                            ...
                            corr_box = box_overlap_with_wrap(
                                b_test, b_set, traj.nx, traj.ny
                            )
    ...
    def print_matching_object_list(self, master_ref=None, select=None):
        ...
        if master_ref is None:
            master_ref = len(self.family) - 1
        if select is None:
            select = np.arange(0, self.family[master_ref].nobjects, dtype=int)
        ...
        for t_off in range(0, len(mol)):
            ...
            for i in range(0, len(select)):
                ...
                for it_back in range(0, len(matching_objects)):
                    ...
                    if len(rep) > 0:
                        print(
                            "t_off: {0} iobj: {1} it_back: {2} obj: {3}".format(
                                t_off, iobj, it_back, rep
                            )
                        )
        return

    def matching_object_list_summary(
        self, master_ref=None, select=None, overlap_thresh=0.02
    ):
        """
                        ...
                        for obj in matching_objects[it_back][i]:
                            if np.size(obj) > 0:
                                ...
                                if obj not in objlist:
                                    ...
                                    inter = self.refine_object_overlap(
                                        t_off, it_back, iobj, obj, master_ref=master_ref
                                    )
        ...
        ret_dict = {
            "master_ref": master_ref,
            "objects": select,
            "matching_object_summary": mols,
        }
        return ret_dict

    def print_matching_object_list_summary(
        self, master_ref=None, select=None, overlap_thresh=0.02
    ):
        """
        ...
        if select is None:
            select = np.arange(0, self.family[master_ref].nobjects, dtype=int)
        mols_dict = self.matching_object_list_summary(
            master_ref=master_ref, select=select, overlap_thresh=overlap_thresh
        )
        ...
        for t_off in range(0, len(mols)):
            ...
            for i in range(0, len(select)):
                ...
                if len(rep) > 0:
                    print("t_off: {0} iobj: {1} obj: {2} ".format(t_off, iobj, rep))

        return

    def find_linked_objects(self, master_ref=None, select=None, overlap_thresh=0.02):
        """
    ...
    def print_linked_objects(self, master_ref=None, select=None, overlap_thresh=0.02):
        """
        ...
        if select is None:
            select = self.family[master_ref].max_at_ref
        linked_objects = self.find_linked_objects(
            master_ref=master_ref, select=select, overlap_thresh=overlap_thresh
        )
        for iobj, linked_obj in zip(select, linked_objects):
            rep = ""
    ...
    def find_super_objects(self, master_ref=None, overlap_thresh=0.02):
        """
                    ...
                    if np.size(obj) > 0:
                        ...
                        if inter > overlap_thresh:
                            if obj in self.family[master_ref - (t_off + 1)].max_at_ref:
                                objlist.append(
                                    (match_time, obj, int(inter * 100 + 0.5))
                                )
                #                                print(objlist)
        ...
        for newref in range(master_ref, 0, -1):
            ...
            for i, obj in enumerate(sup_obj):
                ...
                if len(obj) == 1:
                    ...
                    new_incomplete_object_ids.append(new_incomplete_objects[-1][-1][1])
            incomplete_object_ids = np.array(new_incomplete_object_ids)
            incomplete_objects = new_incomplete_objects
            select = next_level_objs

        for obj in incomplete_objects:
            super_objects.append(np.array(obj))
        len_sup = []
        for s in super_objects:
            len_sup.append(len(s))
    ...
    def refine_object_overlap(self, t_off, time, obj, mobj, master_ref=None):
        ...
        def extract_obj_as1Dint(fam, master_ref, time, obj):
            ...
            mask, objvar = traj.in_obj_func(
                traj, tr_time, obj_ptrs, **traj.ref_func_kwargs
            )
        ...
        tr1D = extract_obj_as1Dint(self.family, master_ref, time, obj)
        trm1D = extract_obj_as1Dint(
            self.family, master_ref - (t_off + 1), time - (t_off + 1), mobj
        )
...
class Trajectories:
    ...
    def __init__(
        ...
        self.refprof = {"rho": rhoref, "p": pref, "pi": piref, "th": thref}
        ...
        self.trajectory = unsplit_objects(
            trajectory, self.labels, self.nobjects, self.nx, self.ny
        )

        (
            self.data_mean,
            self.in_obj_data_mean,
            self.objvar_mean,
            self.num_in_obj,
            self.centroid,
            self.in_obj_centroid,
            self.bounding_box,
            self.in_obj_box,
        ) = compute_traj_boxes(self, in_obj_func, kwargs=kwargs)
    ...
    def var(self, v):
        ...
        for ii, vr in enumerate(list(self.variable_list)):
            if vr == v:
                break
        if ii != i:
            print("var issue: ", self.variable_list.keys(), v, i, ii)
        return ii
...
def compute_trajectories(
    ...
    print(
        "Computing trajectories from {} to {} with reference {}.".format(
            start_time, end_time, ref_time
        )
    )

    ref_file_number, ref_time_index, delta_t = find_time_in_files(files, ref_time)
    ...
    print(
        "Starting in file number {}, name {}, index {} at time {}.".format(
            ref_file_number,
            os.path.basename(files[ref_file_number]),
            ref_time_index,
            ref_times[ref_time_index],
        )
    )
    ...
    #    print(time_index)
    #    input("Press enter")
    trajectory, data_val, traj_error, traj_times, coords = trajectory_init(
        dataset, time_index, variable_list, deltax, deltay, deltaz, refprof, traj_pos
    )
    #    input("Press enter")
    ref_index = 0
        ...
        if time_index >= 0:
            print(
                "Time index: {} File: {}".format(
                    time_index, os.path.basename(files[file_number])
                )
            )
            trajectory, data_val, traj_error, traj_times = back_trajectory_step(
                dataset,
                time_index,
                variable_list,
                refprof,
            ...
            if file_number < 0:
                ...
                print(
                    "File {} {}".format(
                        file_number, os.path.basename(files[file_number])
                    )
                )
    ...
    while (traj_times[-1] < end_time) and (file_number >= 0):
        ...
        if time_index < len(times):
            ...
            trajectory, data_val, traj_error, traj_times = forward_trajectory_step(
                dataset,
                time_index,
                variable_list,
                refprof,
    ...
    data_val = np.reshape(
        np.vstack(data_val), (len(data_val), len(data_val[0]), np.size(data_val[0][0]))
    )

    print(
        "trajectory: {} {} {}".format(
            len(trajectory[1:]), len(trajectory[0]), np.size(trajectory[0][0])
        )
    )
    print(
        "traj_error: {} {} {}".format(
            len(traj_error[1:]), len(traj_error[0]), np.size(traj_error[0][0])
        )
    )
    #    print np.shape()

    trajectory = np.reshape(
        np.vstack(trajectory[1:]),
        (len(trajectory[1:]), len(trajectory[0]), np.size(trajectory[0][0])),
    )

    traj_error = np.reshape(
        np.vstack(traj_error[1:]),
        (len(traj_error[1:]), len(traj_error[0]), np.size(traj_error[0][0])),
    )
...
def trajectory_init(
    ...
    out = data_to_pos(data_list, varp_list, traj_pos, xcoord, ycoord, zcoord)
...
def forward_trajectory_step(
    ...
    def print_info(kk):
        print("kk = {} Error norm :{} Error :{}".format(kk, mag_diff[kk], diff[:, kk]))
        print("Looking for at this index ", [traj_pos[:, m][kk] for m in range(3)])
        print(
            "Nearest solution for the index", [traj_pos_at_est[kk, m] for m in range(3)]
        )
        print("Located at ", [traj_pos_next_est[kk, m] for m in range(3)])
        return
    ...
    def int3d_dist(pos):
        ...
        out = np.max(
            [0, ndimage.map_coordinates(dist_data, pos, mode="nearest", order=2)]
        )
    ...
    confine_traj_bounds(
        traj_pos_next_est, nx, ny, nz, vertical_boundary_option=vertical_boundary_option
    )
    ...
    while not_converged:
        ...
        if err <= errtol_iter or err >= err_prev:
            not_converged = False
            print("Converged in {} iterations with error {}.".format(niter, err))
        ...
        if niter <= max_iter:
            ...
            if use_point_iteration or niter == 0:
                ...
                inv_gradient[use_grad] = np.clip(dx[use_grad] / df[use_grad], -1, 1)
            ...
            if err > errtol:
                ...
                if np.size(diff[bigerr, :]) > 0:
                    ...
                    for kk in k:
                        ...
                        if debug:
                            print(
                                "Index in source array is {},{},{}".format(lx, ly, lz)
                            )
                        ...
                        xpos = phase(
                            x_real[np.ix_(xr, yr, zr)], x_imag[np.ix_(xr, yr, zr)], nx
                        )
                        ypos = phase(
                            y_real[np.ix_(xr, yr, zr)], y_imag[np.ix_(xr, yr, zr)], ny
                        )
                        ...
                        if debug:
                            print(
                                "Nearest point is ({},{},{},{})".format(
                                    lxm, lym, lzm, dist[lxm, lym, lzm]
                                )
                            )
                            print(
                                xpos[lxm, lym, lzm],
                        ...
                        if debug:
                            print(
                                "New estimate, relative = {} {} ".format(res.x, res.fun)
                            )
...
def unsplit_object(pos, nx, ny):
    ...
    if debug_unsplit:
        print("Shape(pos):", np.shape(pos), "Number of clutsters:", n_clust)
    ...
    if debug_unsplit:
        print("Shape(cluster centres):", np.shape(kmeans.cluster_centers_))
    if debug_unsplit:
    ...
    def debug_print(j):
        print("Main cluster:", main_cluster, "cluster number: ", i, "dist:", dist)
        print("Cluster centres:", kmeans.cluster_centers_)
...
def unsplit_objects(trajectory, labels, nobjects, nx, ny):
    ...
    for iobj in range(0, nobjects):
        if debug_unsplit:
            print("Unsplitting Object: {:03d}".format(iobj))
        #        if iobj == 15 :
        #            debug_unsplit = True
        #        else :
        #            debug_unsplit = False
        ...
        for it in range(0, np.shape(trajectory)[0]):
            if debug_unsplit:
                print("Time: {:03d}".format(it))
            tr = trajectory[it, labels == (iobj), :]
            if ((np.max(tr[:, 0]) - np.min(tr[:, 0])) > nx / 2) or (
                (np.max(tr[:, 1]) - np.min(tr[:, 1])) > ny / 2
            ):
                trajectory[it, labels == iobj, :] = unsplit_object(
                    trajectory[it, labels == iobj, :], nx, ny
                )
                if debug_unsplit:
                    print("New object:", trajectory[it, labels == iobj, :])
    return trajectory
...
def print_boxes(traj):
    ...
    for iobj in range(traj.nobjects):
        ...
        for it in range(np.shape(traj.centroid)[0]):
            ...
            strf = strf + "".join(
                [" {:6.2f}".format(traj.centroid[it, iobj, i]) for i in range(3)]
            )
            strf = strf + "".join(
                [" {:10.6f}".format(traj.data_mean[it, iobj, i]) for i in range(6)]
            )
            print(strf)
    return
