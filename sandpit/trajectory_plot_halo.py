...
#    print("MSE: ",mse_ptr, npts_ptr, z_ptr)
    ...
    # True heights.
    zn = tr.coords['zn']
#    zn = (np.arange(0,np.size(tr.refprof['pi']))-0.5)*tr.deltaz
...
#            m2 = (mean_prop['cloud'][1:,iobj,npts_ptr] >10)
            ...
            zbl = np.interp(mean_prop['pre_cloud_bl'][:,iobj,z_ptr],
                            tr.coords['zcoord'], tr.coords['zn'])
...
def plot_traj_pos(traj, index, fn, save=False) :
    ...
    ax.set_xlim(traj.coords['xcoord'][0]-10, traj.coords['xcoord'][-1]+10)
    ax.set_ylim(traj.coords['ycoord'][0]-10, traj.coords['ycoord'][-1]+10)
...
def gal_trans(x, y, galilean, j, timestep, traj) :
    if galilean[0] != 0 :
        x = ( x - galilean[0]*j*timestep/traj.coords['deltax'] )%traj.nx
    if galilean[1] != 0 :
        y = ( y - galilean[1]*j*timestep/traj.coords['deltay'] )%traj.ny
    return x, y
...
def plot_traj_animation(traj, save_anim=False, anim_name='traj_anim',
                        ...
                        plot_class = None,
                        figsize=(10,6),
                        no_cloud_size = 0.2, halo_size = 1.0, cloud_size = 2.0,
                        field_size = 0.5, fps = 10, with_boxes = False,
                        elev = 0.0, azim=0.0, dist = 10,
                        var = None,
                        ) :
    """
    ...
    halo = "halo_trajectory" in traj.__dict__
...
#    print(files)
    ...
    if np.size(select) > 1 :
        ...
        if halo:
            # print("Found halo data")
            xh = traj.halo_trajectory[j,traj.halo_labels == iobj,0]%traj.nx
            yh = traj.halo_trajectory[j,traj.halo_labels == iobj,1]%traj.ny
            x_min = min(x_min, np.min(xh))
            x_max = max(x_max, np.max(xh))
            y_min = min(y_min, np.min(yh))
            y_max = max(y_max, np.max(yh))
            if galilean is not None :
                for j in range(1, np.shape(traj.trajectory)[0]):
                    xh = traj.halo_trajectory[j,traj.halo_labels == iobj,0]%traj.nx
                    yh = traj.halo_trajectory[j,traj.halo_labels == iobj,1]%traj.ny
                    xh, yh = gal_trans(xh, yh,  galilean, j, timestep, traj)
                    x_min = min(x_min, np.min(xh))
                    x_max = max(x_max, np.max(xh))
                    y_min = min(y_min, np.min(yh))
                    y_max = max(y_max, np.max(yh))
        ...
        if np.isin(iobj,select) :
            if plot_class is None :
                ...
                line_cl, = ax.plot([], [], linestyle='' ,marker='o', \
                                   ...
                                   label='{}'.format(iobj))
                if halo:
                    line_hl, = ax.plot([], [], linestyle='' ,marker='+', \
                                       markersize = halo_size, \
                                       color = line.get_color())
                    line_list.append([line, line_cl, line_hl])
                else:
                ...
                if halo:
                    line_hl, = ax.plot([], [], linestyle='' ,marker='+', \
                                       ...
                                       color = 'brown',
                                       label='HALO')
                    line_for_class_list.append(line_hl)
...
#                print(f'Plotting object {iobj}')
                ...
                if halo:
                    # print("Found halo data")
                    xh = traj.halo_trajectory[j,traj.halo_labels == iobj,0]
                    yh = traj.halo_trajectory[j,traj.halo_labels == iobj,1]
                    zh = traj.halo_trajectory[j,traj.halo_labels == iobj,2]
                    if galilean is not None :
                        xh, yh = gal_trans(xh, yh,  galilean, j, timestep, traj)

                    # xh = conform_plot(xh, traj.nx, xlim)
                    # yh = conform_plot(yh, traj.ny, ylim)
                ...
                if plot_class is None :
                    ...
                    not_in_cl = ~in_cl
                    if "halo_trajectory" in traj.__dict__:
                        [line, line_cl, line_hl] = line_list[nplt]
                        line_hl.set_data(xh, yh)
                        line_hl.set_3d_properties(zh)
                    else:
                    ...
                    for (iclass, line) in enumerate(line_list[nplt]) :
                        if iclass < len(line_list[nplt])-1:
...
#    writer = Writer(fps=15, metadata=dict(artist='Me'), bitrate=1800)
    ...
    if fps > 0 :
        ...
        if save_anim : anim.save(anim_name+anim_type, fps=fps, dpi=dpi)#, extra_args=['-vcodec', 'libx264'])
    else :
...
def plot_traj_family_members(traj_family, selection_list, galilean = None, \
    ...
    for selection in selection_list :
        ...
        ax.set_zlim(0, tr.coords['zcoord'][-1])
        if galilean is not None :
