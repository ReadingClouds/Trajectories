...
#                  "q_cloud_liquid_mass":[False,False,False],\
#                  "buoyancy":[False,False,False],\
#                  "q_total":[False,False,False],\
#                  "th_L":[False,False,False],\
#                  "qt_p":[False,False,False],\
#                  "thL_p":[False,False,False],\
#                  }

#var_properties = {"w":[False,False,True],\
#                  "th":[False,False,False],\
#                  "p":[False,False,False],\
#                  "q_vapour":[False,False,False],\
#                  "q_cloud_liquid_mass":[False,False,False],\
...
#                  "q_cloud_liquid_mass":[False,False,False],\
#                  "buoyancy":[False,False,False],\
#                  "wpbstep":[False,False,True],\
#                  "wpdstep":[False,False,True],\
#                  "wpstep":[False,False,True],\
#                  "wbuoy":[False,False,True],\
#                  "w_eff_buoy":[False,False,True],\
#                  "w_budget_tend":[False,False,True],\
#                  "w_total_tend":[False,False,True],\
#                  "w_pwadv":[False,False,True],\
#                  }
...
var_properties = {"u":[True,False,False],\
                  ...
                  "q_vapour":[False,False,False],\
                  "q_cloud_liquid_mass":[False,False,False],\
                  "q_rain_mass":[False,False,False],\
                  "q_ice_mass":[False,False,False],\
                  "q_snow_mass":[False,False,False],\
                  "q_graupel_mass":[False,False,False],\
                  "buoyancy":[False,False,False],\
                  "wpbstep":[False,False,True],\
                  "wpdstep":[False,False,True],\
                  "wpstep":[False,False,True],\
...
class Trajectories :
    ...
    def __init__(self, files, ref_prof_file, start_time, ref, end_time,
        ...
        if variable_list == None :
            ...
            #      "tracer_rad1":r"$trad_{1}$ kg/kg", \
            #      "tracer_rad2":r"$trad_{2}$ kg/kg", \
            #      }
            #variable_list = { \
            #      "u":r"$u$ m s$^{-1}$", \
            #      "v":r"$v$ m s$^{-1}$", \
...
### start Jian-Feng Gu, 22th Sep, 2020 ###
        self.data, trajectory, self.traj_error, self.times, self.ref, \
        self.labels, self.nobjects, self.coords, self.deltat, \
        self.halo_data, self.halo_trajectory, self.halo_traj_error, \
        self.halo_labels, self.halo_dist_labels = \
        compute_trajectories(files, start_time, ref, end_time,
                             deltax, deltay, deltaz, variable_list.keys(),
                             self.refprof,
                             ref_func, kwargs=kwargs)
### end Jian-Feng Gu, 22th Sep, 2020 ###
...
### start Jian-Feng Gu, 22th Sep, 2020 ###
    #traj_pos, labels, nobjects, halo_pos, halo_labels, halo_dist_labels = ref_func(dataset, time_index, **kwargs)
    traj_pos, labels, nobjects, halo_pos, halo_labels, halo_dist_labels = ref_func(dataset, time_index, refprof, **kwargs)
    #if ref_func == 'trajectory_cloud_ref':
    #    traj_pos, labels, nobjects, halo_pos, halo_labels, halo_dist_labels = ref_func(dataset, time_index, **kwargs)
    #else:
    #    # following is for buoyant cloud updraft identification
    #    traj_pos, labels, nobjects, halo_pos, halo_labels, halo_dist_labels = ref_func(dataset, time_index, refprof, **kwargs)
### end Jian-Feng Gu, 22th Sep, 2020 ###
...
### start Jian-Feng Gu, 22th Sep, 2020 ###
    halo_trajectory, halo_data_val, halo_traj_error, halo_traj_times, halo_coords \
      = trajectory_init(dataset, time_index, variable_list,
                        deltax, deltay, deltaz, refprof, halo_pos)
### end Jian-Feng Gu, 22th Sep, 2020 ###
...
### start Jian-Feng Gu, 22th Sep, 2020 ###
            print('Now for cloud halo backward trajectory')
            halo_trajectory, halo_data_val, halo_traj_error, halo_traj_times = \
            back_trajectory_step(dataset, time_index, variable_list, refprof,
                  halo_coords, halo_trajectory, halo_data_val, halo_traj_error, halo_traj_times)
### end Jian-Feng Gu, 22th Sep, 2020 ###
            ref_index += 1
        else :
            file_number -= 1
...
### start Jian-Feng Gu, 22th Sep, 2020 ###
            print('Now for cloud halo forward trajectory')
            halo_trajectory, halo_data_val, halo_traj_error, halo_traj_times = \
                forward_trajectory_step(dataset, time_index,
                                 variable_list, refprof,
                                 halo_coords, halo_trajectory, halo_data_val, halo_traj_error,
                                 halo_traj_times, vertical_boundary_option=2)
### end Jian-Feng Gu, 22th Sep, 2020 ###
        else :
            file_number += 1
...
### start Jian-Feng Gu, 22th Sep, 2020 ###
    halo_data_val = np.reshape(np.vstack(halo_data_val), \
                    (len(halo_data_val), len(halo_data_val[0]), np.size(halo_data_val[0][0]) ) )

    halo_trajectory = np.reshape(np.vstack(halo_trajectory[1:]), \
                      ( len(halo_trajectory[1:]), len(halo_trajectory[0]), \
                      np.size(halo_trajectory[0][0]) ) )

    halo_traj_error = np.reshape(np.vstack(halo_traj_error[1:]), \
                      ( len(halo_traj_error[1:]), len(halo_traj_error[0]), \
                      np.size(halo_traj_error[0][0]) ) )

    halo_traj_times = np.reshape(np.vstack(halo_traj_times),-1)
### end Jian-Feng Gu, 22th Sep, 2020 ###
...
### start Jian-Feng Gu, 22th Sep, 2020 ###
    return data_val, trajectory, traj_error, traj_times, ref_index, \
      labels, nobjects, coords, delta_t, halo_data_val, halo_trajectory, \
      halo_traj_error, halo_labels, halo_dist_labels
### end Jian-Feng Gu, 22th Sep, 2020 ###
...
##############################################
### Edited by Jian-Feng Gu, 1st June, 2021 ###
##############################################

    #print('maxindex', maxindex)
    #print('use_corrected_positions', use_corrected_positions)
    #print('output', output)
    #print('x, num_x', x[0], len(x))
    #print('num_data',data[3], data[3].shape, len(data))
...
#                t += w
                for l in range(maxindex) :
                    ...
                    #print(i,j,k)
                    #print(max(II), min(II), max(JJ), min(JJ), max(KK), min(KK), l, len(data), type(data[l]))
                    #print(II.shape, JJ.shape, KK.shape, data[l].shape)
                    #print(data[l][II,JJ,KK].shape, output[l].shape, II.shape, JJ.shape, KK.shape, WX[i].shape, WY[j].shape, WZ[k].shape)
                    #print(l, varp_list[l], output[l], output[l].shape,II, JJ, KK, WX[i], WY[j], WZ[k])
...
def load_traj_pos_data(dataset, it) :
    ...
    if cyclic_xy :
        ...
        #varp_list = [var_properties['buoyancy']]*5
        ...
        #varp_list = [var_properties['buoyancy']]*3
...
#                                            refprof, coords)
            ...
            vard = theta * (1 + q_v/epsilon) / (1 + q_v + q_cl)
            ...
            qt, varp = get_data(source_dataset, 'q_total', it, refprof, coords)
            mean_qt = np.mean(qt, axis = (0,1))
            vard = qt - mean_qt

        elif var_name == "thL_p":

            thL, varp = get_data(source_dataset, 'th_L', it, refprof, coords)
            mean_thL = np.mean(thL, axis = (0,1))
            vard = thL - mean_thL

        elif var_name == "wpstep":

            #print('w_pstep')
            var     = source_dataset["tend_w_pstep_3d_local"]
            w_pstep = var[it, ...]
            varp    = var_properties[var_name]

            mean_wpstep = np.mean(w_pstep, axis = (0,1))
            vard = w_pstep - mean_wpstep

        elif var_name == "wbuoy":

            #print('w_buoy')
            var1    = source_dataset["tend_w_pstep_3d_local"]
            w_pstep = var1[it, ...]
            vp      = var_properties["wpstep"]

            var2    = source_dataset["tend_w_buoyancy_3d_local"]
            w_buoy  = var2[it, ...]
            varp    = var_properties["wbuoy"]

            mean_wpstep = np.mean(w_pstep, axis = (0,1))
            vard = w_buoy + mean_wpstep

        elif var_name == "wpbstep":
            ...
            var2     = source_dataset["w_pbstep"]
            w_pbstep = var2[it, ...]
            varp     = var_properties["wpbstep"]

            mean_wpstep = np.mean(w_pstep, axis = (0,1))
            vard = w_pbstep - mean_wpstep

        elif var_name == "wpdstep":

            #print('w_pdstep')
            var2    = source_dataset["w_pdstep"]
            vard    = var2[it, ...]
            varp    = var_properties["wpdstep"]

        elif var_name == "w_eff_buoy":

            #print('w_eff_buoy')
            var1    = source_dataset["tend_w_pstep_3d_local"]
            w_pstep = var1[it, ...]
            vp1     = var_properties["wpstep"]

            var2     = source_dataset["w_pbstep"]
            w_pbstep = var2[it, ...]
            vp2      = var_properties["wpbstep"]

            mean_wpstep = np.mean(w_pstep, axis = (0,1))
            wpbstep  = w_pbstep - mean_wpstep

            var3    = source_dataset["tend_w_buoyancy_3d_local"]
            w_buoy  = var3[it, ...]
            varp    = var_properties[var_name]

            vard    = wpbstep + w_buoy

        elif var_name == "w_budget_tend":
            ...
            var3    = source_dataset["tend_w_damping_3d_local"]
            w_damp  = var3[it, ...]

            var4    = source_dataset["tend_w_viscosity_3d_local"]
            w_vis   = var4[it, ...]

            var6    = source_dataset["tend_w_pwadvection_3d_local"]
            w_pw    = var6[it, ...]

            varp    = var_properties[var_name]
            vard    = w_pstep + w_buoy + w_damp + w_vis + w_pw

        elif var_name == "w_pwadv":
        ...
        elif var_name == "u_pstep":

            #print('u_pstep')
            var     = source_dataset["tend_u_pstep_3d_local"]
            vard    = var[it, ...]
            varp    = var_properties[var_name]

        elif var_name == "v_pstep":

            #print('v_pstep')
            var     = source_dataset["tend_v_pstep_3d_local"]
            vard    = var[it, ...]
            varp    = var_properties[var_name]

        elif var_name == "w_total_tend":

            #print('w_total_tend')
            var1    = source_dataset["tend_w_total_3d_local"]
            vard    = var1[it, ...]
            varp    = var_properties[var_name]
...
###### start 19th Feb, 2022, Jian-Feng Gu ##################

def _test_indices_3d(i, j, k, k_limit, k_start, diagonal=False, extended=False):
    if extended:
...
###### end 19th Feb, 2022, Jian-Feng Gu ####################
...
def trajectory_cloud_ref(dataset, time_index, thresh=0.00001) :
    ...
    @author: Peter Clark and Jian-Feng Gu
...
### start, Jian-Feng Gu, 22th Sep, 2020 ###
    ...
    halo_labels, halo_dist_labels, halo_logic_pos = label_halo(data, labels, nobjects, nlay=dist_num, threshold=0.00001)

    halo_labels = halo_labels[halo_logic_pos]
    halo_dist_labels = halo_dist_labels[halo_logic_pos]

    halo_pos = np.where(halo_logic_pos)

    traj_halo_pos = np.array( [xcoord[halo_pos[0][:]], \
                          ycoord[halo_pos[1][:]], \
                          zcoord[halo_pos[2][:]]],ndmin=2 ).T
...
### start, Jian-Feng Gu, 22th Sep, 2020 ###
    ...
    return traj_pos, labels, nobjects, traj_halo_pos, halo_labels, halo_dist_labels

### end, Jian-Feng Gu, 22th Sep, 2020 ###

### start, Jian-Feng Gu, 19th Feb, 2021 ###
###    buoyant cloud identification     ###
def trajectory_buoyant_cloud_ref(dataset, time_index, refprof, thresh=0.00001) :
    """
    Function to set up origin of back and forward trajectories.
    ...
    Args:
        ...
        time_index     : Index of required time in file.
        thresh=0.00001 : Cloud liquid water threshold for clouds.
        refprof        : Dict with reference theta profile arrays. Added by Jian-Feng Gu, 12th Feb, 2021
...
#    print(thresh)
    ...
    ### buoyancy ###
    theta    = dataset.variables["th"][time_index, ...]
    theta   += refprof['th']
    q_v      = dataset.variables["q_vapour"][time_index, ...]
    q_cl     = data

    thetav   = theta * (1 + q_v/epsilon) / (1 + q_v + q_cl)
    thetav_m = np.mean(thetav, axis = (0,1))
    buoy     = thetav - thetav_m

    logical_buoy = cloud_select(buoy, thresh=0.0)

    ### mask the buoyant cloud ###
    logical_pos  = np.logical_and(logical_pos, logical_buoy)
...
### end, Jian-Feng Gu, 19th Sep, 2021 ###

### start, Jian-Feng Gu, 19th Feb, 2021  ###
### buoyant cloud updraft identification ###
def trajectory_buoyant_cloud_up_ref(dataset, time_index, refprof, thresh=0.00001) :
    """
    Function to set up origin of back and forward trajectories.
...
#    print(thresh)
    ...
    ### updraft ###
    ww      = dataset.variables["w"][time_index, ...]
    nz      = ww.shape[2]
    ww_zn   = np.zeros_like(ww)
    ww_zn[:,:,1:nz] = 0.5*(ww[:,:,0:nz-1]+ww[:,:,1:nz])
    ww_zn[:,:,0]    = ww[:,:,0]

    logical_up = cloud_select(ww_zn, thresh=0.0)

    ### mask the buoyant cloud updraft ###
    logical_buoy_up = np.logical_and(logical_buoy, logical_up)
    logical_pos  = np.logical_and(logical_pos, logical_buoy_up)
...
### start, Jian-Feng Gu, 19th Feb, 2021  ###
###     cloud updraft identification     ###
def trajectory_cloud_up_ref(dataset, time_index, refprof, thresh=0.00001) :
    """
    Function to set up origin of back and forward trajectories.
...
#    print(thresh)
    ...
    ### mask the cloud updraft ###
    logical_pos  = np.logical_and(logical_pos, logical_up)
...
### start, Jian-Feng Gu, 19th Feb, 2021  ###
### buoyant cloud updraft identification ###
def trajectory_buoyant_cloud_up_deep_ref(dataset, time_index, refprof, thresh=0.00001) :
    """
    Function to set up origin of back and forward trajectories.
...
#    print(thresh)
    ...
    qr  = dataset.variables["q_rain_mass"][time_index, ...]
    qi  = dataset.variables["q_ice_mass"][time_index, ...]
    qs  = dataset.variables["q_snow_mass"][time_index, ...]
    qg  = dataset.variables["q_graupel_mass"][time_index, ...]
    #data = qcl + qr + qi + qs + qg
    data = qcl + qi + qs + qg
    ...
    logical_up = cloud_select(ww_zn, thresh=0.0)
    #logical_up = cloud_select(ww_zn, thresh=1.0)
    ...
    print('Setting labels.')
    #labels, nobjects = label_3D_cyclic(mask)
    nobjects, labels = label_clds_3d(mask, diagonal=True, min_cells=5)
    labels = labels - 1
    nobjects = nobjects - 1

    print('Halo labels')
    halo_labels, halo_dist_labels, halo_logic_pos = label_halo_all(data, labels, nobjects, nlay=dist_num, threshold=0.00001)
...
### start, Jian-Feng Gu, 22th Sep, 2020 ###
def label_halo_all(data, labels, nobjects, nlay=5, threshold=0.00001):
    """
    ...
    Args:
        ...
        nlay=5         : number of layers for halo region
        thresh=0.00001 : Cloud liquid water threshold for clouds.
    ...
    @author: Jian-Feng Gu and Peter Clark
...
def label_halo(data, labels, nobjects, nlay=5, threshold=0.00001):
    ...
    for iobj in range(0, nobjects):
        ...
        for n in range(0,nlay):
            ...
            layer[temp] = False
            layer = np.logical_and(layer, data<threshold)
            # label the halo region of object iobj with its own object ID
            halo_labels[layer] = iobj
            # encode the distance of halo away from the cloud boundary
            halo_dist_labels[layer] = np.exp(n+1)
            temp = cld_extend_logic
